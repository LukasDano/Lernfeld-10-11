...
export const DropDownSelect: FC<DropDownSelectProps> = ({ items, selectedItem, onChange, minWidth }) => {
    return (
        <div className="flex-shrink-0 w-fit" style={{ minWidth: minWidth ? `${minWidth}px` : '150px' }}>
            <Listbox value={selectedItem} onChange={onChange}>
               ...
               py-2 pl-3 pr-10 text-left shadow-md border
               dark:shadow-gray-700 dark:border-gray-600
               focus:outline-none focus:ring-2 focus:ring-blue-500 sm:text-sm"
                    ...
                    <ListboxOptions
                        className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-md
                           bg-white dark:bg-gray-500 py-1 text-base shadow-lg
